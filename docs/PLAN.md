...
- 17 September 2026: Play Billing — `hearts_small` (5 hearts), `hearts_large` (20) and `remove_ads`. Bought hearts go above the cap, Remove ads stops the banner and interstitials but not the rewarded ad, purchases are acknowledged on completion, and the entitlement stays out of the backup code (it is shareable). The shop keeps its «قريباً» row until Play returns the products, so this ships before the payments profile is verified.
...
### Release & tech debt
- **P0 · S · owner**: Finish the payments profile (under verification on 17 September 2026), then create the three products in Play Console with ids `hearts_small`, `hearts_large`, `remove_ads`, set their prices, and add a licence tester to buy without paying. Nothing sells until this is done.
