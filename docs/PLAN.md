...
- v1.0.6 — redesign foundation: the new pitch background, `Surface`/`StatusPill`, the three buttons, section headings, rows cards and progress bars, from the owner's Claude Design project. Screens follow one PR at a time.
- v1.0.6 — owner's request, 14 September 2026: the result screen no longer shows the answer recap after a level (kept after quick play and the daily; reporting a question stays in the quiz's feedback panel).
